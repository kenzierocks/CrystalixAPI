 ...
 * 
 * An edge is specified by the id numbers to the 2 nodes that are to be joined.
 * Each edge is directed i.e. one-way, so to create a bidirectional (two-way)
 * link between the nodes requires two edges to be created. This does have the
 * advantage that each the cost of travelling between 2 nodes does not have to
 * be the same in both directions. <br>
 * 
 * It is more efficient to add all the nodes first and then the edges but not
 * essential. <br>
 * 
 * Attempting to add an edge where one or both of the connecting nodes do not
 * yet exist in the graph will be 'remembered' - this is called a floating edge.
 * Once both nodes have been added to the graph then the floating edge will also
 * be added to the graph. <br>
 * Floating edges are segregated from the graph edges to simply the graph
 * searching algorithms. <br>
 * <br>
 * 
 * This arrangement is very flexible and can simplify the code needed to create
 * the graph at the expense of creating large numbers of floating edges that
 * will never be added to the graph. Once you have created the final graph it is
 * recommended that the user calls the compact method which simply deletes any
 * floating edges and requests a garbage collection to release the memory. <br>
 * <br>
 * 
 * The classes
 * 
 * @see GraphNode
 * @see GraphEdge are the base classes used to store nodes and edges. These
 *      classes support inheritance so you can provide more specialised classes
 *      for your graphs. <br>
 * 
...
public class Graph {
    ...
    /**
     * Create a graph with an initial capacity based on an estimate of the
     ...
     * update all floating edges and where possible adding the floating edge to
     * the graph.
     * 
     * @param node
     *            a node to be added to the graph (must not be null)
     */
    ...
    /**
     * If the node exists remove it and all edges that start or end at this
     * node.
     * 
     * @param nodeID
    ...
    /**
     * Locate and return the first node encountered that is within a stated
     * distance of a position at [x,y,z]
     * 
     * @param x
     * @param y
     * @param z
     * @param maxDistance
     *            only consider a node that is with this distance of [x,y,z]
     ...
     * 
     * @param fromID
     *            the ID number of the from node
     * @param toID
     *            the ID number of the to node
     * @param cost
     *            cost from > to
    ...
    public boolean addEdge(int fromID, int toID, double cost) {
        ...
        FloatingEdge floatEdge = new FloatingEdge(fromID, toID, fromNode,
                toNode, cost);
        if (fromNode == null)
     ...
     * @return true if the edge was added else false
     */
    public boolean addEdge(int fromID, int toID, double costOutward,
            double costInward) {
        boolean added = false;
     ...
     * If either node does not exist or there is no edge exists between them
     ...
     * If either node does not exist or there is no edge exists between them
     * then the method returns a value <0.
     * 
     * @param fromID
     *            ID for the from node
     * @param toID
     ...
     * This will delete the edge from one node to another but does not remove
     * any return edge. <br>
     * To remove a 'bidirectional route' between nodes 22 and 33 then you must
     * call this method twice e.g. <code>
     ...
     * 
     * @param nodeID
     *            id of the node where the edges start from
     */
     ...
     * 
     * @param node
     *            the node where the edges start from
     */
    ...
    /**
     * Will return an array of all the GraphEdges in the graph. <br>
     * The type of each element in the array will be of type Object if the
     * parameter is null otherwise it is T (where T is GraphEdge or any class
     * derived from GraphEdge.
     * 
     * @param <T>
     * @param array
     *            a zero length array of GraphNode or any derived class.
     */
     ...
     * Will return an array of all the GraphEdges that start from the node. <br>
     * The type of each element in the array will be of type Object if the
     * parameter is null otherwise it is T (where T is GrahEdge or any class
     * that extends GrahEdge.
     * 
     * @param <T>
     * @param from
     *            the node where the edges start from
     ...
     * Will return an array of all the GraphNodes in the graph. <br>
     * The type of each element in the array will be of type Object if the
     * parameter is null otherwise it is T (where T is GraphNode or any class
